...
def collate_fn(batch):
    ...
    batch_entity_weight = [b[0]['entity_weight'] for b in batch]
    batch_motion_weight = [b[0]['motion_weight'] for b in batch]
    ...
    batch_data = {
        ...
        'batch_entity_weight':
        nn.utils.rnn.pad_sequence(batch_entity_weight, batch_first=True).float(),
        'batch_motion_weight':
        nn.utils.rnn.pad_sequence(batch_motion_weight, batch_first=True).float(),
    }
...
from datasets.charadesvgg import CharadesVGG
from datasets.charadesCD import CharadesCD
from datasets.activitynetCD import ActivityNetCD
