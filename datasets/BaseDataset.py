...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, split):
        ...
        self.anno_dirs = {}
        self.anno_dirs["Charades"] = "data/charades"
        self.anno_dirs[
            "ActivityNet"
        ] = "/dev/shm/activitynet/captions"
        self.anno_dirs["TACoS"] = "/dev/shm/tacos"
        self.feature_dirs = {}
        self.feature_dirs["Charades"] = "data/charades/new"
        self.feature_dirs[
            "ActivityNet"
        ] = "/dev/shm/activitynet/my_i3d_feature"
        # /home/yinjiong/dataset/new_dataset/activitynet
        # /dev/shm/activitynet/my_i3d_feature
        self.feature_dirs["TACoS"] = "data/tacos/org"

        self.anno_dirs["Charades-CD"] = "data/charades/charades_cd"
        self.feature_dirs["Charades-CD"] = "data/charades/new"

        self.anno_dirs[
            "ActivityNet-CD"
        ] = "data/activitynet/captions/CD"
        # /dev/shm/activitynet/my_i3d_feature  
        self.feature_dirs[
            "ActivityNet-CD"
        ] = "data/activitynet/my_i3d_feature"

        self.anno_dirs[
            "DiDeMo"
        ] = "data/didemo/annotations"
        self.feature_dirs[
            "DiDeMo"
        ] = "data/didemo/features"
    ...
    def __getitem__(self, index):
        ...
        entity_weight = self.annotations[index]['entity_weight']
        motion_weight = self.annotations[index]['motion_weight']
        ...
        entity_weight = torch.tensor(entity_weight, dtype=torch.float)
        motion_weight = torch.tensor(motion_weight, dtype=torch.float)
        # print(1/entity_weight)
        ...
        if not (pos_tags.size() == word_idxs.size() == entity_weight.size() == motion_weight.size()):
            print('sentence:', sentence)
            print('pos_tags', pos_tags.size())
            print('word_idxs', word_idxs.size())
            print('entity_weight', entity_weight.size())
            print('motion_weight', motion_weight.size())

        assert (
            pos_tags.size() == word_idxs.size() == entity_weight.size() == motion_weight.size()
    ...
    def get_video_features(self, vid, dataset):
        if dataset != 'DiDeMo': # and 'ActivityNet':
            try:
            ...
            features = np.array(self.videos[vid])
            features = np.squeeze(features)
            features = torch.from_numpy(features).float()  # [T,dim]
        ...
        frame_rate = 3 if dataset != ("Charades" or "Charades-CD") else 1
