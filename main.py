...
import datasets
import models
from models.HDT.loss import aligment_score, early_pred_loss, compute_loss, L1_aligment_score, intra_constrast_loss
from models.HDT.eval import eval_test
...
def iterator(dataset_name, split):
    ...
    elif split == "test" or split == "test_iid" or split == "test_ood":
        test_dataset = getattr(datasets, dataset_name)(split)
        dataloader = DataLoader(
            test_dataset,
            batch_size=config.TEST.BATCH_SIZE,
            shuffle=False,
...
if __name__ == "__main__":
    ...
    if args.mode.lower() == "train":
        ...
        for epoch in range(config.TRAIN.MAX_EPOCH):
            ...
            for data, anno in dataloader_train:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # l1_loss = l1_loss1 + l1_loss2

                    # cons_loss1 = intra_constrast_loss(output[4], batch_vis_mask, batch_internel_label)
                    # cons_loss2 = intra_constrast_loss(output[6], batch_vis_mask, batch_internel_label)
                    # cons_loss = cons_loss1 + cons_loss2

                    loc_loss, match_loss = compute_loss(
                        start_logits,
                        end_logits,
                        additional_logits,
                        batch_start_frame,
                ...
                if global_step % 50 == 0:
                    logger.info(
                        ...
                        # "epoch: {}, step: {}/{}, lr: {:.6f},  total: {:.4f}, loc: {:.4f}, match: {:.4f}, early: {:.4f}".format(
                        # "epoch: {}, step: {}/{}, lr: {:.6f},  total: {:.4f}, loc: {:.4f}, early: {:.4f}".format(
                        # "epoch: {}, step: {}/{}, lr: {:.6f},  total: {:.4f}, loc: {:.4f}, kl/l1: {:.6f}, early: {:.4f}".format(
                            epoch + 1,
                            step,
                            step_epoch,
                ...
                # writer.add_scalar("Loss_train/l1", l1_loss, global_step)
                ...
                # writer.add_scalar("Loss_train/cons", cons_loss, global_step)
                writer.add_scalar(
                    "learning_rate",
                    optimizer.state_dict()["param_groups"][0]["lr"],
                    global_step,
            ...
            if config.TEST.EVAL_TRAIN:
                # trainset eval
                r1i3, r1i5, r1i7, mi, score_str, statistics_str = eval_test(
                    model=model,
            ...
            # testset eval
            tb = PrettyTable()
            tb.field_names = ["dataset", "epoch", "spilt", "r1i3", "r1i5", "r1i7", "miou"]
            test_datasets = ["Charades", "ActivityNet", "TACoS"]
            test_split = ['test']
            # print(config.DATASET.NAME[-2:])
            if config.DATASET.NAME != "Combine":
                test_datasets = [config.DATASET.NAME]
            if config.DATASET.NAME[-2:] == 'CD':
                test_split = ['test_iid', 'test_ood']
                #Q print("if no test split")
            ...
            for test_set in test_datasets:
                for split in test_split:
                    r1i3, r1i5, r1i7, mi, score_str, statistics_str = eval_test(
                        model=model,
                        data_loader=iterator(test_set, split),
                        device=device,
                        mode=split,
                        epoch=epoch + 1,
                    )
                    ...
                    # logger.info("\n" + statistics_str)
                    writer.add_scalar(
                        "Accuracy_test/{}_{}_r1i3".format(test_set, split), r1i3, epoch + 1
                    )
                    writer.add_scalar(
                        "Accuracy_test/{}_{}_r1i5".format(test_set, split), r1i5, epoch + 1
                    )
                    writer.add_scalar(
                        "Accuracy_test/{}_{}_r1i7".format(test_set, split), r1i7, epoch + 1
                    )
                    writer.add_scalar(
                        "Accuracy_test/{}_{}_miou".format(test_set, split), mi, epoch + 1
                    )
                    tb.add_row(
                        [
                            test_set,
        ...
        filename = "/mnt/data/yinjiong/bridge/results/Charades/bridge_T_split_num_1/checkpoints/HMDN_35.t7"
        print("using ->{}<- ...".format(filename))

        # model.load_state_dict(torch.load(filename))n
        ...
        with torch.no_grad():
            ...
            for idx, batch_data in enumerate(dataloader):
                ...
                print(torch.sigmoid(start_logits.squeeze()).cpu().numpy(), start_logits.shape)
                pred['fg_pred'] = np.array(torch.sigmoid(output[5].squeeze()).cpu().numpy()).tolist()
                # np.save('./pred/'+ vid + '_' + str(idx), torch.sigmoid(output[5].squeeze()).cpu().numpy())
                pred['fg_shape'] = output[5].shape
                ...
                pred['description'] = [annos[i]["description"] for i in range(len(annos))]
                ...
                for (
                    ...
                    pred['pred_start'] = np.array(start_time).tolist()
                    print("pred_end:", end_time)
                    pred['pred_end'] = np.array(end_time).tolist()

                    pred_list.append(pred)

            with open('./pred/'+ "pred_" + vid + '.json', 'w') as f:
